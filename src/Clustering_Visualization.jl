...
using CSV, DataFrames, MLJ, MLJLinearModels, Random, Distributions, MLJMultivariateStatsInterface, Plots, MLJClusteringInterface, Distances
...
mdenoise = fit!(machine(PCA(maxoutdim = 2), all_clean_uncorrelated_train_input), verbosity = 0); #fitting a PCA machine for denoising the training data
report(mdenoise) #reporting the main characteristics of the PCA denoising machine
cleaned_train_input_PCA_visualization = MLJ.transform(mdenoise, all_clean_uncorrelated_train_input) #transforming our training dataset into a 5000X2 DataFrame (only keep 2 predictors/genes for the clustering)

clustering_visualization = scatter(cleaned_train_input_PCA_visualization[:,1], cleaned_train_input_PCA_visualization[:,2], legend = false, c = Int.(int(all_train_data_output)), title = "Clustering Visualization", xlabel = "Gene 1", ylabel = "Gene 2") #plotting the clusters of the training data
